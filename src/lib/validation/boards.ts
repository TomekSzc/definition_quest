...
export const GenerateBoardSchema = z.object({
  ...
  inputText: z.string().min(1, "Input text cannot be empty").max(5000, "Input text must not exceed 5,000 characters"),
  ...
  tags: z
    .array(z.string().max(20, "Each tag must not exceed 20 characters").trim())
...
export const CreateBoardSchema = z
  .object({
    ...
    tags: z
      .array(z.string().min(1, "Tags cannot be empty").max(20, "Each tag must not exceed 20 characters").trim())
...
// Base object schema (before .transform) so we can reuse & omit keys
const ListBoardsBaseSchema = z.object({
  ...
  q: z.preprocess((v) => (typeof v === "string" && v.trim() === "" ? undefined : v), z.string().max(100)).optional(),
  tags: z
    .preprocess(
      (v) => {
  ...
  sort: z
    ...
    .default("created"),
  direction: z.preprocess((v) => (v === undefined ? "desc" : v), z.enum(["asc", "desc"])).default("desc"),
});

/**
 * Validation schema for listing public boards with pagination, filtering and sorting.
 */
export const ListBoardsSchema = ListBoardsBaseSchema.transform((data) => ({
  ...data,
...
/**
 * Validation schema for listing boards the current authenticated user has played.
 * Same as ListBoards but without ownerId filter.
 */
export const ListPlayedBoardsSchema = ListBoardsBaseSchema.omit({ ownerId: true }).transform((data) => ({
  ...data,
  page: Number(data.page),
  pageSize: Number(data.pageSize),
}));
