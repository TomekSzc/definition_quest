...
export class OpenRouterService {
   ...
   */
  public async chatCompletion(messages: Message[], options?: Partial<ChatOptions>): Promise<ChatCompletion> {
    // Validate messages
   ...
   */
  private async sendRequest(endpoint: string, body?: Record<string, unknown>, timeoutMs = 30000): Promise<Response> {
  ...
  private async handleChatResponse(response: Response, responseFormat?: JsonSchemaFormat): Promise<ChatCompletion> {
    ...
    if (responseFormat) {
      try {
        ...
        throw new SchemaValidationError("Response does not match expected JSON format", error);
      }
    }
    ...
    for (let attempt = 0; attempt < this.maxRetries; attempt++) {
      try {
        ...
        if (error instanceof RateLimitError || error instanceof ServiceUnavailableError) {
          ...
          this.log("warn", `Retrying after ${delayMs}ms (attempt ${attempt + 1}/${this.maxRetries})`, {
            error: error.message,
          });
