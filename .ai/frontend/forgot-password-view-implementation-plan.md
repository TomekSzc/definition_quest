...
- **Typy**:
  ...
  - `ForgotPasswordFormState` _(ViewModel)_:
    ```ts
...
| Sukces odpowiedzi                       | Toast _success_, opcjonalnie route push do `/signin`. |
| Błąd sieci                              | Toast _error_.                                        |
...
2. **Błąd sieci/serwera** – toast _error_ z ogólnym komunikatem.
...
8. **QA/a11y**:
   - `aria-label` na input.
   - Możliwość wysłania formularza klawiszem _Enter_.
