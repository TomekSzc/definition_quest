...
2. **baseUrl** _(opc.)_ – adres bazowy API; pozwala na łatwe mockowanie w testach.

## 3. Publiczne metody i pola

| Metoda                               | Zwraca                    | Opis                                                  |
| ------------------------------------ | ------------------------- | ----------------------------------------------------- |
| `chatCompletion(messages, options?)` | `Promise<ChatCompletion>` | Wysyła pełne zapytanie _chat/completions_.            |
...
| `retry<T>(fn)`                         | Prostolinijna logika _exponential back-off_ dla 429/5xx.  |
...
2. Dodaj globalny _error boundary_ (middleware) do mapowania Error → HTTP.
...
const completion = await service.chatCompletion([sysMsg, userMsg], {
  model,
  responseFormat,
  ...params,
});
